...
(function ($) {
    ...
    appConfig = { 'topLevelFolderId': '5368188c7bee043b2d028f8c', // solaris
                  ...
                  'jobString': "#!/bin/bash%0A%0A/home/scott/projects/ParaView/build-make-gpu/bin/pvpython /home/scott/projects/ParaView/src/Web/Python/pv_web_wavelet.py -f --data-dir /home/scott/Documents/cosmodata/anldemo --port 8000 --pipeline WaveletPipeline --host localhost"
                  // 'jobString': "#!/bin/bash%0A%0A/home/scott/projects/ParaView/build-make-gpu/bin/pvpython /home/scott/projects/ParaView/build-make-gpu/lib/site-packages/paraview/web/pv_web_visualizer.py --content /home/scott/projects/ParaView/build-make-gpu/www -f --data-dir /home/scott/Documents/cosmodata/anldemo --port 8000"
                  // 'jobString': "#!/bin/bash%0A%0A/home/scott/projects/ParaView/build-make-fresh/bin/pvpython /home/scott/projects/ParaView/build-make-fresh/lib/site-packages/paraview/web/pv_web_visualizer.py --content /home/scott/projects/ParaView/build-make-fresh/www -f --data-dir /home/scott/Documents/cosmodata/anldemo --port 8000"
                },
    ...
    token = null,
    imgFilePattern = new RegExp('(^[^-]+)-(halos-spheres|MagnitudeDensity|PointDensity)-(x|y|z)\\.png'),
    filePathPattern = new RegExp('anldemo\/(.+)$'),
    overviewPngUrl = null,
            ...
            if (statusString === 'running') {
                ...
                if (readyBtn.hasClass('fresh')) {
                    ...
                    readyBtn.bind('click', function(event) {
                        ...
                        var connectUrl = "ws://" + hostnamePrefix + hostnameSuffix +
    ...
    CosmoSummaryView = Backbone.View.extend({
        ...
        events: {
            ...
            'click .launch-pvweb-button-ptsvolren': 'launchPtsVolViz',
            'click .launch-pvweb-button-magvolren': 'launchMagVolViz',
            'click .launch-pvweb-button-halos': 'launchPtsViz',
        },
        ...
        launchPtsViz: function(e) {
            ...
            launchFunction(this.model.attributes.halosFile,
                           this.model.attributes.hostname);
        },

        // Called to launch paraviewweb to visualize the volume data
        launchPtsVolViz: function(e) {
            launchFunction(this.model.attributes.pointDensityFile,
                           this.model.attributes.hostname);
        },

        // Called to launch paraviewweb to visualize the volume data
        launchMagVolViz: function(e) {
            launchFunction(this.model.attributes.magnitudeDensityFile,
                           this.model.attributes.hostname);
        }
    }),
    ...
    /*
     * Convenience function to trigger the action that will cause the
     * vtk:exit rpc method to get called, which in turn will close the
     * connection and end the remote process.
     */
